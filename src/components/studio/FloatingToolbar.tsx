              ...
              <div className="flex flex-wrap gap-0.5">
                {shadowOptions.map(o => (
                  <button key={o.value} onClick={() => onUpdateEffects({ shadowDepth: o.value })}
                    className={`px-2 py-0.5 rounded-full text-[9px] font-medium transition-colors ${element.effects.shadowDepth === o.value ? 'bg-primary text-primary-foreground' : 'bg-secondary text-secondary-foreground hover:bg-accent border border-border'}`}>
                    {o.label}
                  </button>
                ))}
              </div>
